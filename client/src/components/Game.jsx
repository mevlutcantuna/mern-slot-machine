...
const Game = () => {
  ...
  const play = () => {
    ...
    // if incOrDescCoin func does not work correctly,it shows error message
  ...
  useEffect(() => {
    ...
    if (gameStart) {
      setSlotValue1({
        backgroundImage: `url(${Reel1})`,
        backgroundRepeat: "no-repeat",
        backgroundPosition: `0 -${90 * s1}px`,
      });
      setSlotValue2({
        backgroundImage: `url(${Reel2})`,
        backgroundRepeat: "no-repeat",
        backgroundPosition: `0 -${90 * s2}px`,
      });
      setSlotValue3({
        backgroundImage: `url(${Reel3})`,
        backgroundRepeat: "no-repeat",
        backgroundPosition: `0 -${90 * s3}px`,
      });
    }
    ...
    if (
      ...
      !gameStart &&
        dispatch(incOrDescCoin(user?.coins + 3, user?._id, 3, gameStart));
    }
  ...
  return (
    <div className="game">
      ...
      <div className="game__slots">
        <div
          style={!gameStart ? slotValue1 : {}}
          className={`game__slots__slot ${gameStart && "slot1-active"}`}
        />
        <div
          style={!gameStart ? slotValue2 : {}}
          className={`game__slots__slot ${gameStart && "slot2-active"}`}
        />
        <div
          style={!gameStart ? slotValue3 : {}}
          className={`game__slots__slot ${gameStart && "slot3-active"}`}
        />
      </div>
      <button disabled={gameStart} onClick={play} className="game__play">
        {gameStart ? "spinning" : "play"}
      </button>
    </div>
  );
};
